...
var STORAGE_KEY_EMAIL = '@AsyncStorageKey:signupEmail';
...
export default class SignupVriew extends Component {
  ...
  _loadInitialState = async() => {
    try {
      var value = await AsyncStorage.getItem(STORAGE_KEY_EMAIL);
  ...
  register() {
    if (this.state.userName == '' || this.state.email == '' || this.state.password == '') {
      ...
      //存储用户输入的邮箱
      this._onValueChange(this.state.email);
    }
  }

  _onValueChange = async(email) => {
    this.setState({email});
    try {
